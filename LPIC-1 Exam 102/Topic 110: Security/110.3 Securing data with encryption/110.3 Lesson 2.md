...
Команда для работы с GPG - `gpg`. Вы можете передать ему несколько вариантов для выполнения разных задач. Давайте начнем с создания пары ключей в качестве пользователя `carol`. Для этого вы воспользуетесь командой `gpg --gen-key`:
